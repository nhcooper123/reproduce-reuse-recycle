...
clean_data_all <-
  ...
  mutate(data_format = case_when(data_format == ".csv/.tsv;" ~ ".csv/.tsv",
                                 ...
                                 data_format == "Unclear" ~ "Unsure",
                                 data_format == ".csv/.tsv;.RData" ~ ".csv/.tsv;.Rdata",
                                 data_format == ".xls(x);fastq" ~ ".xls(x);.fastq",
                                 data_format == ".csv/.tsv;.xls(x);fastq.gz" ~ ".csv/.tsv;.xls(x);.fastq.gz",
                                 data_format == ".xls(x);fastq.gz" ~ ".xls(x);.fastq.gz",
                                 TRUE ~ as.character(data_format))) %>%
  ...
  #------------------------------------------------------------------------------------------------------
  # 25. Does the code have a CITATION file? This variable was added at the last minute and probably needed
  # more explanation that this is a specific file contained within a code archive. 
  # Although several comments point to other ways this info has been included, I am classing these as "No"
  # Note that this variable is likely of limited use for broader analyses due to confusion.                                                                                                    
  mutate(code_CITATION = case_when(code_CITATION == "'How to cite the package?' section in README" |
                                   code_CITATION == "Both data and code are archived using the same DOI"  |                                                                
                                   code_CITATION == "citation available in zenodo" |                                                                                       
                                   code_CITATION == "citation available on zenodo"   |                                                                                     
                                   code_CITATION == "Citations are given in the README" |                                                                                  
                                   code_CITATION == "Citations included in README" |                                                                                       
                                   code_CITATION == "Code was submitted along with raw data. Both have same ciatation information." |                                      
                                   code_CITATION == "Has citation on the wbesite but not in a file format."   |                                                           
                                   code_CITATION == "Included in the data citation" |                                                                                      
                                   code_CITATION == "It has a citation field on Zenodo" |                                                                                  
                                   code_CITATION == "It has equivalent Zenodo field"  |                                                                                    
                                   code_CITATION == "Mentioned in readme"  |                                                                                               
                                   code_CITATION == "No but the README gives two papers to cite if the package is used"  |                                                 
                                   code_CITATION == "No file in R package named CITATION but there is a DESCRIPTION and citation instructions are generated by citation()"|
                                   ...
                                   code_CITATION == "Unclear" 
                                   ~ "Unsure",
                                  
                                   code_CITATION == "No code archived"  
                                   ~ NA_character_,
                                  
                                   TRUE ~ as.character(code_CITATION))) %>%
  
  #------------------------------------------------------------------------------------------------------
  # 26. Can you download the code?
  # Code that do not have a link cannot be downloaded so make this NA
  mutate(code_download = case_when(code_link == "No"  | is.na(code_link) ~ NA_character_,                                                                                               
                                   TRUE ~ as.character(code_download))) %>%
  # These can be classed as Yes/No.
  mutate(code_download = case_when(code_download == "Broken link" |
  ...
  #------------------------------------------------------------------------------------------------------
  # 26. Can you open the code?
  # Code that cannot be downloaded cannot be opened so make this NA
  mutate(code_open = case_when(code_download == "No"  | is.na(code_download) ~ NA_character_,                                                                                               
                                 TRUE ~ as.character(code_open))) %>%
  # Mostly these are versions of "Yes" or "No". Technically even if code needs to be copy-pasted from a PDF this is Yes
  mutate(code_open = case_when(code_open == ".R files described in ReadMe are not included in the zipfile" |
                               code_open == "Broken link" |
                               code_open == "Yes, but what I can see is not intelligible"
                                 ~ "No",
                               ...
                               code_open == "we are able to open the Python code but not Matlab" |
                               code_open == "The program is for Windows and I'm running on a Mac"
